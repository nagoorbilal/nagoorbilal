...
function WeeklyReport(props) {
  ...
  const customerRef =useRef(null)
    ...
    const [collectionRpt,setcollectionRpt] = useState([])
    ...
    const [CustomerData, setCustomerData] = useState([])
       ...
       // Table Header
   let columnIndex = ["தேதி", "பாக்கி தொகை", "வரவுத்தொகை"]; 
   // Table Row
   const Items = ["collection_date","opening_balance","collection_amount"]; 
   // Excel Export
   const exportData = [
    {
      columns: columnIndex,
  ...
  const getColumnSearchProps = (dataIndex,Column, disabled ) => ({
   ...
   const searchText = (confirm, vals,column) => {
    ...
    let newData = collectionRpt.filter((val)=> val.customer_name == vals); 
    if(newData.length > 0) {
      setcollectionRpt(newData)
      confirm();
      return false;
    }else{
        collectionReportApi();
   ...
   const columns = [
    {
      title: () => {return (
        <div>
            {"Date"}
            {(collectionRpt.length > 0) &&
                  <Tooltip title={"Sort"} placement="top"> 
                      <ImportExportIcon className="tableSort"
                        onClick={() => {                                               
                          let data = collectionRpt.sort(CompareValues('collection_date',((SortType === "asc") ? "desc" : "asc")));                                        
                          setTimeout(function(){ setcollectionRpt(data)}, 1000);                         
                        }}
                      />
                  </Tooltip>
            }
      ...
      className: "left",
     ...getColumnSearchProps('Date',"collection_date",""),
    },
    {
        title: () => {return (
          <div>
              ...
              {(collectionRpt.length > 0) &&
                    <Tooltip title={"Sort"} placement="top"> 
                        <ImportExportIcon className="tableSort"
                          onClick={() => {                                               
                            let data = collectionRpt.sort(CompareValues('opening_balance',((SortType === "asc") ? "desc" : "asc")));                                        
                            setTimeout(function(){ setcollectionRpt(data)}, 1000);                         
                          }}
                        />
                    </Tooltip>
      ...
      },
    {
          ...
          <div>
              ...
              {(collectionRpt.length > 0) &&
                    <Tooltip title={"Sort"} placement="top"> 
                        <ImportExportIcon className="tableSort"
                          onClick={() => {                                               
                            let data = collectionRpt.sort(CompareValues('collection_amount',((SortType === "asc") ? "desc" : "asc")));                                        
                            setTimeout(function(){ setcollectionRpt(data)}, 1000);                         
                          }}
                        />
                    </Tooltip>
        ...
        // width:"20%",
        // editable: true,
       ...getColumnSearchProps('Collection Amount',"collection_amount","disabled"),
      },
...
const custNameChange = (val) =>{
    ...
    const collectionReportApi = () => {
      if(!Customer.Id){
        ...
        setLoading(true)
        let url = config.Api+"collectionreport"
        let data = {
        ...
        .then(res => {  
            ...
            if(res.Output.status.message == "Failed"){
              ...
              if(res.Output.data.collection_report.length){ 
                console.log(res.Output.data.collection_report)
               setcollectionRpt(res.Output.data.collection_report)
            }else{
                setcollectionRpt([])
                setMessage({  open: true, color: "error", message: "Data Not Found" });
            }
  ...
  const getAllDataPrint = () => {
    ...
    let url = config.Api+"collectionreport"
      ...
      setLoading(false); 
        if(res.Output.data){ 
            let result = res.Output.data.collection_report; 
            if(result.length > 0){
              setPrintData(res.Output.data.collection_report)
              let exportItem=[];  

              res.Output.data.collection_report.map((value) => {
                exportItem.push([
           ...
           <CardBody>
            ...
            <div className="add-export-btn" style={{top: "55px"}}>  
                    <Export_Print 
                      ...
                      Refresh={()=>{getCustomerData();setcollectionRpt([]);setPrintData([]);setExportData([]);collectionReportApi()}}
                      Exceldisabled={false}
                      Printdisabled={false}
                      ref={ref}
                    />
               ...
               />   
                   ...
                   <Typography className="totalRecords" gutterBottom>
                       {"Total"} {collectionRpt.length} {"Records"}
                   </Typography>
               </GridItem> 
               </GridContainer>
