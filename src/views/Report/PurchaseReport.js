...
const custNameChange = (val) =>{
    ...
    if(val != null){
        setCustomer({Id: val.pur_customer_idpk,Code: val.customer_code, Name: val.customer_name})
    ...
    const SalesReportApi = () => {
      if(!Customer.Id){
        ...
        setLoading(true)
        let url = config.Api+"purchasereport"
        let data = {
        ...
        .then(res => {  
            ...
            if(res.Output.status.message == "Failed"){
              ...
              var arr = res.Output.data.purchase_report[0];
              var arr2 = res.Output.data.purchase_report[1].collections_details;

              console.log('arr',arr.length,'arr2',arr2.length)
              
              setSalesRpt(res.Output.data.purchase_report[0]);
              logPrints(res.Output.data.purchase_report[0]);
    ...
    let url = config.Api+"purchasereport"
      ...
      setLoading(false); 
        if(res.Output.data){ 
            ...
            // if(result.length > 0){
              setPrintData(Object.values(res.Output.data.purchase_report[0]))
              
              ref.current.PrintValue(res.Output.data.purchase_report[0])
              return false;
    ...
    let url = config.Api+"purchasereport"
      ...
      setLoading(false); 
        if(res.Output.data){ 
          ...
          var collection = res.Output.data.purchase_report[1].collections_details;
          let arrcollect = [];
          if(collection.length > 0){
            ...
            setTotcollectAmt(arrcollect.reduce((a, b) => a + b, 0))
              ...
              setPrintData(Object.values(res.Output.data.purchase_report[0]))
              refNew.current.PrintValue(res.Output.data.purchase_report[0])
  ...
  const tableBody =  stateValues.map((state, index) => {
    ...
    const itemRows = itemValues.map((item, i) => {
      ...
      const dates =
        ...
        const customer =
        i === 0 ? (
          <td rowSpan={itemValues.length + 1}>{state.pur_customer_name}</td>
        ) : null;
