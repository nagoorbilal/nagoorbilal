...
function CustomerRegistry(props) { 
  ...
  /*----------------------For using Redux store and useState----------------------*/
    ...
    const isEditing = record => record.pur_customer_idpk === EditingKey; 
  ...
  useEffect(()=>{
   ...
   const searchText = (confirm, vals,column) => {
    ...
    let newData = content.PurCustomerData.filter((val)=> val.pur_customer_idpk == vals); 
    if(newData.length > 0) {
      dispatch({type:'PurCustomerData', data:newData});
      confirm();
      return false;
    }else{
      ...
      <div>
        {
          ...
          <Autocomplete
          options={content.PurCustomerData}
          getOptionLabel={(option) => option[Column]}
          id="debug"          
          onChange={(e,value)=>{searchText(confirm, value == null ? '' :value['pur_customer_idpk'], Column)}}
  ...
  //    Column option for table
    const PriceCategcolumns = [{
        title: () => {return (
          <div>
              {"Customer Code"}
              {(content.PurCustomerData.length > 0) &&
                    <Tooltip title="Sort" placement="top"> 
                        <ImportExportIcon className="tableSort"
                          onClick={() => {                                               
                            let data = content.PurCustomerData.sort(CompareValues('customer_code',((SortType === "asc") ? "desc" : "asc")));                                        
                            setTimeout(function(){ dispatch({type:'PurCustomerData', data:data});}, 1000);                         
                          }}
                        />
                    </Tooltip>
              }
          ...
          <div>
              ...
              {(content.PurCustomerData.length > 0) &&
                    <Tooltip title="Sort" placement="top"> 
                        <ImportExportIcon className="tableSort"
                          onClick={() => {                                               
                            let data = content.PurCustomerData.sort(CompareValues('customer_name',((SortType === "asc") ? "desc" : "asc")));                                        
                            setTimeout(function(){ dispatch({type:'PurCustomerData', data:data});}, 1000);                         
                          }}
                        />
                    </Tooltip>
          ...
          <div>
              ...
              {(content.PurCustomerData.length > 0) &&
                    <Tooltip title="Sort" placement="top"> 
                        <ImportExportIcon className="tableSort"
                          onClick={() => {                                               
                            let data = content.PurCustomerData.sort(CompareValues('Place',((SortType === "asc") ? "desc" : "asc")));                                        
                            setTimeout(function(){ dispatch({type:'PurCustomerData', data:data});}, 1000);                         
                          }}
                        />
                    </Tooltip>
          ...
          <div>
              ...
              {(content.PurCustomerData.length > 0) &&
                    <Tooltip title="Sort" placement="top"> 
                        <ImportExportIcon className="tableSort"
                          onClick={() => {                                               
                            let data = content.PurCustomerData.sort(CompareValues('mobile_no',((SortType === "asc") ? "desc" : "asc")));                                        
                            setTimeout(function(){ dispatch({type:'PurCustomerData', data:data});}, 1000);                         
                          }}
                        />
                    </Tooltip>
        ...
        <div>
            ...
            {(content.PurCustomerData.length > 0) &&
                  <Tooltip title="Sort" placement="top"> 
                      <ImportExportIcon className="tableSort"
                        onClick={() => {                                               
                          let data = content.PurCustomerData.sort(CompareValues('opening_balance',((SortType === "asc") ? "desc" : "asc")));                                        
                          setTimeout(function(){ dispatch({type:'PurCustomerData', data:data});}, 1000);                         
                        }}
                      />
                  </Tooltip>
  ...
  },
{
  title: "Action",
  width: "10%",
  dataIndex: "pur_customer_idpk",
  render: (pur_customer_idpk, record)=>{
      ...
      return editable ? (
          <div>
            ...
            onConfirm={() => {onCancel(record.pur_customer_idpk)}}>
          ...
          <Tooltip title="Edit" placement="top"> 
            ...
            <Tooltip title="Delete" placement="top"> 
                <IconButton component="div" className="padding0px" onClick={()=>deleteAlert(record.pur_customer_idpk)}
                disableRipple={true}>
                  <DeleteIcon  className="DeleteIcon" />
                  </IconButton>
            </Tooltip>
...
function CompareValues(key, order) {
  return function innerSort(a, b) {  
     setLoading(true);       
      dispatch({type:'PurCustomerData', data:[]});     
      setSortType(order);
 ...
 const saveSubmit = (data) => {
  ...
  if(content.CustomerID){
    setLoading(true)
    const url = config.Api + "purchasecustupdate";
      const SubmitParam = {
        "purchasecustregistry":[
        {
        ...
        "pur_customer_idpk": content.CustomerID
    ...
    setLoading(true)
    const url = config.Api + "purchasecust_save";
      const SubmitParam = {
        "purchase_cust_registry":[
        {
        "customer_code": window.CustomerCode,
        "customer_name": window.CustomerName,
        "mobile_no": window.MobileNo,
  ...
  const CustomerDelete = (id) => {
    ...
    const url = config.Api + "purchasecustdelete";
     console.log("id",id)
      fetch(url+'/'+ id, {
        method: "Post",
        headers: {
        ...
        if(res.Output.status.code == 200){ 
          ...
          if(res.Output.status.message == "No Record"){
            ...
            if(result.length > 0){
                ...
                dispatch({ type: 'PurCustomerData', data: result})
  ...
  const clearDispatch = () => {
    dispatch({type: 'PurCustomer_Edit', id: '',active:'1'});
   ...
   const addNewrow = () =>{ 
    if(EditingKey == 'new'){
    const result = content.PurCustomerData.filter(val => val.pur_customer_idpk == 'new'); 
    if(result.length == 0){            
        const newArr = {
            "key": content.PurCustomerData.length + 1 ,
            "pur_customer_idpk": 'new',
            "CustomerCode": '',
            "CustomerName": '',
            "Place": '',
            "MobileNo": '',
        ...
        const data = [newArr].concat(content.PurCustomerData);
        dispatch({type:'PurCustomerData', data:data});
...
const editRecord = (record) =>{
  ...
  const arr = content.PurCustomerData.find(val => val.pur_customer_idpk == 'new');    
  if(arr){
    setMessage({
          open: true,
          color: "warning",
    ...
    setEditingKey(record.pur_customer_idpk);
    dispatch({type: 'PurCustomer_Edit', id: record.pur_customer_idpk,active: record.IsActive });
...
const onCancel = (id) =>{        
  if(id == 'new'){
      const newArr = content.PurCustomerData.filter(val => val.pur_customer_idpk != id);
      dispatch({type: 'PurCustomerData', data: newArr});
      clearDispatch()
  }else{
      setEditingKey('new');
      clearDispatch()
    ...
    const getInput = (record) => { 
      ...
      return(
        ...
        return(
          ...
          onClick={(val)=>{
            dispatch({type: 'PurCustomer_Edit', id: content.CustomerID,active: val})
          }}
          />
        )
      }
  ...
  return(
      <div>
          ...
          <Table
              ...
              columns={columns} 
              dataSource={content.PurCustomerData} 
