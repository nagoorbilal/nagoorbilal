...
const deleteAlert = (key) => {
  ...
  const submitConfirmation = () => {
    ...
    if(DetailTableData.length == 0){
      setMessage({ open: true, color: "error", message: "Please Add the Items" });
      return false;
...
const blurCollection = () => {
   if(CollectionAmt){
    // setNetAmt(parseFloat(parseFloat(parseFloat(CurrAmt) + parseFloat(defaultOpeningBal) ) - parseFloat(CollectionAmt)))
