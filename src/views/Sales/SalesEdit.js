...
import Sales_Print from '../../components/_helperComponents/SalesPrint'
...
function Sales(props) {
  ...
  const custCodeRef = React.useRef();
    ...
    const [OldOpenBal, setOldOpenBal] = useState('')
  ...
  useEffect(() => {
    dispatch({ type: "customerBillInfo", data : [] })
    finalClear();
    //getCustomerData();
    getItemData();
  ...
  useEffect(() => {
        ...
        geteditRecord(content.CustBilldata);
    localStorage.setItem('BillRefresh', false);    
    
    clear();
  },[content.CustBilldata])


  const geteditRecord = (data) =>{
    ...
    setSalesIDPK(data[0].sales_idpk)
    setBillNo(data[0].bill_no)
    

    setDetailTableData(data[0].customer_bill_find)

    setBillDate(data[0].bill_date.split("-").reverse().join("-"))

    let reAssign = [];
    data[0].customer_bill_find.map((val,index) => {
      reAssign.push({
        "salesDetIDPK": val.sales_det_idpk,
        "SNo": index + 1 ,
        "ItemIDPK": val.item_id,
        "ItemCode": val.item_code,
        "ItemName": val.item_name,
        "CustoemrIDPK": data[0].customer_id,
        "CustomerCode": data[0].customer_code,
        "CustomerName": data[0].customer_name,
        "UoMName": val.qty + " "+ quantityType(val.uom),
        // "UoMName": val.uom == '1' ? val.qty +" "+"Kg" : val.qty+" "+"Box",
        "UoM": val.uom,
        "Qty": val.qty,
        "Rate": val.rate,
        "BillDate":val.BillDate,
        "Amount":val.amount,
        // "Amount": parseFloat(val.qty * parseInt(val.rate)).toFixed(2),
        "key": index + 1,
        "status": "0"
      })
    ...
    setCustomer({ Id: data[0].customer_id, Code: data[0].customer_code,
         Name: data[0].customer_name, Openbal: data[0].opening_balance})

    setDetailTableData(reAssign)


    setOldOpenBal(data[0].opening_balance)
    setLessAmt(data[0].less_amount)
    setOpenAmt(data[0].opening_balance)
    setCurrAmt(data[0].total_amount)
    setBckCurrAmt(parseFloat(data[0].total_amount) + parseFloat(data[0].less_amount))

    let netamt = parseFloat(parseFloat(data[0].old_opening_balance) + parseFloat(data[0].total_amount) - parseFloat(data[0].commission))
    setNetAmt(parseFloat(netamt - data[0].less_amount).toFixed(2));
    setDefaultOpeningBal(data[0].old_opening_balance)
...
const clear = () => {
 ...
 const editRecord = (data) => {
    ...
    console.log("setSno",data.BillDate)
...
const deleteAlert = (key) => {
    ...
    if(salesUpdate.length > 0 && SalesIDPK){
      ...
      console.log("salesUpdate",salesUpdate)
      ...
      var editData = new Object();
        ...
        editData = { 
            ...
            "UoM": updteSal.UoM,
            "UoMName": updteSal.Qty + " "+ quantityType(updteSal.UoM),
            // "UoMName": updteSal.UoM == '1' ? updteSal.Qty +" "+"Kg" : updteSal.Qty +" "+"Box",
            "Qty": updteSal.Qty,
            "Rate": updteSal.Rate,
  ...
  const saveAlert = (e) =>{
    setConfirmationAlert(
        <SweetAlert
          ...
          title={"Are you sure?"}                  
            onConfirm={() => { saveConsumerBill(eval)}}
 ...
 const saveCollectionAlert = () =>{
  setConfirmationAlert(
    ...
    const saveSubmit = async() => {
        ...
        setLoading(true)
        const url = config.Api + "salessave";
    ...
    const collectOnlysave = () => {
      setConfirmationAlert(null);
        ...
        // setLoading(true)
        const url = config.Api + "collectionssave";
  ...
  const submitDetConfirmation = () => {
    ...
    const itemCode = itemArr.filter(item => item.ItemName === Item.Code);
    const itemName = itemArr.filter(item => item.ItemName === Item.Name);
  ...
  const custNameChange = (val) =>{
    ...
    if(val != null){
        itemRef.current.focus();
        setCustomer({Id: val.customer_idpk,Code: val.customer_code, Name: val.customer_name, Openbal: val.opening_balance})
        setOpenAmt(val.opening_balance);
        setNetAmt(parseFloat(OpenAmt) + parseFloat(CurrAmt))
        setOldOpenBal(val.opening_balance)
  ...
  const saveConsumerBill = async(e) => {
    ...
    // Status = 2 ->insert
        ...
        setLoading(true)
        const url = config.Api + "customer_bill_save";

        var selesDet = []
    await  DetailTableData.map(val=> {
           selesDet.push({
                sales_det_idpk: val.salesDetIDPK,
                item_id: val.ItemIDPK,
          ...
          .then(res => {
            ...
            if (response.code == "200") {
              if(response.message == "Success"){
                ...
                document.getElementById("tabSalesEdit").getElementsByTagName("span")[1].click()
                return false;
  ...
  const handlelessAmt = (val) => {
    ...
    if(val.length == 0){
      ...
      if(Commission){
        setNetAmt(parseFloat(defaultOpeningBal) + parseFloat(CurrAmt) - parseFloat(Commission))
        return false;
    }else{
        setNetAmt(parseFloat(defaultOpeningBal) + parseFloat(CurrAmt))
        return false;
    }
      ...
      if(Commission){
        let netAmt = parseFloat(defaultOpeningBal) + parseFloat(CurrAmt - val) - parseFloat(Commission);
          setNetAmt(parseFloat(netAmt).toFixed(2));
          setLessAmt(val)  
      }else{
        let netAmt = parseFloat(defaultOpeningBal) + parseFloat(CurrAmt - val);
  ...
  const handlelCommission = (val) => {
    ...
    if(val.length == 0){
      if(LessAmt){
        setNetAmt(parseFloat(defaultOpeningBal) + parseFloat(CurrAmt) - parseFloat(LessAmt)); 
        return false;
      }else{
        setNetAmt(parseFloat(defaultOpeningBal) + parseFloat(CurrAmt))
        return false;
      ...
      if(LessAmt){
        console.log('defaultOpeningBal',defaultOpeningBal,'CurrAmt',CurrAmt)
        let netAmt = parseFloat(defaultOpeningBal) + parseFloat(CurrAmt - val) - parseFloat(LessAmt);
        setNetAmt(parseFloat(netAmt).toFixed(2));
        setCommission(val)
  ...
  const collectionCalc = (val) => {
    ...
    if(val.length == 0){
     setNetAmt(parseFloat( parseFloat(CurrAmt) + parseFloat(defaultOpeningBal) ));
    }
}
...
const blurCollection = () => {
   ...
   if(CollectionAmt){
    setNetAmt(parseFloat( parseFloat(CurrAmt) + parseFloat(defaultOpeningBal))  - parseFloat(CollectionAmt))
   }
...
function isValidDate(d) {
  console.log('d',d,d instanceof Date && !isNaN(d))
...
<SubmitBtn  
                     ...
                     <ThermalPrtSales 
                         ...
                         Items={Items}
                         data={DetailTableData.filter(item=> item.status !== "1")}
                ...
                <GridContainer>
                  ...
                  <GridItem sm={3} md={3} xs={3} lg={3}></GridItem>
                  <GridItem sm={3} md={3} xs={3} lg={3} style={{padding:"0px !important"}}>
                  <Textbox placeholder={"Less Amount"}
                        type="number"
                        value={LessAmt}
...
export default withStyles(sweetAlertStyle)(Sales);
