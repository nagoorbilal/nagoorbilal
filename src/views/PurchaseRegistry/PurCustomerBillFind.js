...
function CustomerBillFind(props) {
  ...
  const getColumnSearchProps = (dataIndex,Column, disabled ) => ({
      ...
      <div>
        {
          ...
          onChange={(e,value)=>{searchText(confirm, value == null ? '' :value['purchase_idpk'], Column)}}
   ...
   const searchText = (confirm, vals,column) => {
    ...
    let newData = CustomerBillFindRpt.filter((val)=> val.purchase_idpk == vals); 
    if(newData.length > 0) {
   ...
   const columns = [
      ...
      },
    {
      ...
      },
      {
        title: "Action",
        width: "10%",
        dataIndex: "purchase_idpk",
        render: (purchase_idpk, record)=>{
         return   (
                <div>
                   
                <Tooltip title="Edit" placement="top"> 
                        <IconButton component="div" className="padding0px" onClick={()=>editRecord(purchase_idpk)}
                          disableRipple={true}>
...
function CompareValues(key, order) {
  ...
  const editRecord = (id) => {
    let getData = CustomerBillFindRpt.filter((val)=> val.purchase_idpk == id); 

    localStorage.setItem('BillRefresh', true); 

    dispatch({ type: 'PurchaseBillInfo', data : getData})
    // dispatch({ type: 'CusBillRefresh', CusBillRefresh: true})
    document.getElementById("TriggerNewDoc").value = 'PurchaseEdit'; 
    var link = document.getElementById('TriggerNewDoc'); 
    link.click('PurchaseEdit');
  }
...
const custNameChange = (val) =>{
    ...
    const SalesBill_Api = () => {
      if(!Customer.Id){
        ...
        setLoading(true)
        let url = config.Api+"purchase_bill_find"
        let data = {
