...
function PurchseEdit(props) {
  ...
  useEffect(() => {
    dispatch({ type: "PurchaseBillInfo", data : [] })
    finalClear();
    //getCustomerData();
    getItemData();
  ...
  useEffect(() => {
        ...
        geteditRecord(content.PurchaseBilldata);
    localStorage.setItem('BillRefresh', false);    
    
    clear();
  },[content.PurchaseBilldata])
  ...
  const geteditRecord = (data) =>{
    ...
    setSalesIDPK(data[0].purchase_idpk)
    setBillNo(data[0].bill_no)
    

    setDetailTableData(data[0].purchase_bill_find)

    setBillDate(data[0].bill_date.split("-").reverse().join("-"))

    let reAssign = [];
    data[0].purchase_bill_find.map((val,index) => {
      reAssign.push({
        "salesDetIDPK": val.pur_det_idpk,
        "SNo": index + 1 ,
 ...
 const saveCollectionAlert = () =>{
  ...
  const saveConsumerBill = async(e) => {
    ...
    // Status = 2 ->insert
        ...
        setLoading(true)
        const url = config.Api + "pur_customer_bill_save";
          ...
          .then(res => {
            ...
            if (response.code == "200") {
              if(response.message == "Success"){
                ...
                document.getElementById("tabPurchaseEdit").getElementsByTagName("span")[1].click()
                return false;
...
export default withStyles(sweetAlertStyle)(PurchseEdit);
